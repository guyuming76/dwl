...
monitor="${1}"
# 对于单显示器来说,直接用${1}, 也就是config_dwl里面传进来的参数空字符串方便些,不论啥wlr-randr output名称都不用改
# 我在 https://github.com/Alexays/Waybar/issues/147 看到多显示器设置,但问题是不知道当点击某个显示器上的标签时,把显示器名称从config_dwl传到当前脚本;
# 一个想法就是在config-dwl里为每个output写一组配置,output名称写死在文件里,在使用wlr-randr 配置显示的时候,动态改config_dwl文件内容.但总觉得这样不好
